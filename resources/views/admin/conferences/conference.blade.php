        ...
        <div class="data">
            <div class="row mt-5">
                <div class="offset-md-3 col-12 col-md-9 col-12">
                    <div class="heading">
                        <div class="row">
                            <div class="col-6"><h2>Conferences</h2></div>
                            <div class="col-6">
                                <a href="{{route('conference.create')}}"><div class="button btn">Create</div></a>
                    ...
                    <div class="table-responsive data_table">
                        ...
                        <table class="table table-striped">
                          ...
                          <tbody>
                            ...
                            <tr>
                                <th scope="row">{{($conferences->currentPage() - 1) * $conferences->perPage() + $loop->iteration}}</th>
                                <th>{{$c->title}}</th>
                                <th>{{$c->field}}</th>
                                <td>{{date('F j, Y', strtotime($c->deadline))}}</td>
                                <td>
                                    <a href="{{route('conference.show',$c->id)}}"><span class="badge badge-pill badge-info">Details</span></a>
                                    <a href="{{route('conference.edit',$c->id)}}"><span class="badge badge-pill badge-warning">Update</span></a>
                                    <form action="{{route('conference.destroy', $c->id)}}" method="post" style="display:inline;">
                                        @method('delete')
                                        @csrf
