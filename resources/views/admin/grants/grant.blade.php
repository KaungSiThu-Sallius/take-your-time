                            ...
                            <table class="table table-striped">
                                ...
                                <tbody>
                                    @foreach ($grants as $g)
                                        <tr>
                                            <th scope="row">
                                                {{ ($grants->currentPage() - 1) * $grants->perPage() + $loop->iteration }}
                                            </th>
                                            <th>{{ $g->title }}</th>
                                            <th>{{ $g->level }}</th>
                                            <td>{{ date('F j, Y', strtotime($g->deadline)) }}</td>
                                            <td>
                                                <a href="{{ route('grant.show', $g->id) }}"><span
                                                        class="badge badge-pill badge-info">Details</span></a>
                                                <a href="{{ route('grant.edit', $g->id) }}"><span
                                                        class="badge badge-pill badge-warning">Update</span></a>
                                                <form action="{{ route('grant.destroy', $g->id) }}" method="post"
